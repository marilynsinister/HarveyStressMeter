...
namespace HarveyStressMeter.Testing
{
    ...
    internal static class StressMcpTools
    {
        public static readonly object[] All =
        {
            ...
            Tool("stress_thunder_debug",
                "Read-only thunder flashback snapshot (stabilization, grace, relapse chance, causes)."),
            Tool("stress_thunder_force_relapse",
                "Debug: force TryRollThunderRelapse (warning + light/heavy relapse path).",
                Prop("ignore_chance", "boolean", "Skip random roll (default true)")),
            Tool("stress_thunder_stabilize_harvey",
                "Debug: apply StabilizeWithHarvey (grace period + hidden sensitivity).",
                Prop("stress_decay", "integer", "StressLoad decay amount (default 15)"),
                Prop("grace_minutes", "integer", "Harvey anchor grace minutes (default 120)"),
                Prop("reason", "string", "Optional log label")),
            Tool("stress_thunder_clear", "Reset thunder/relapse/sensitivity flashback state."),
            ...
            Tool("stress_darkness_step1_progress",
                "Debug: set step1 progress (evenings completed + today minutes).",
                Prop("evenings", "integer", "Completed evenings (0–3)"),
                Prop("today", "integer", "Today's progress minutes (0–60)"),
            ...
            Tool("stress_darkness_status", "Read-only step1 therapy status + darkness snapshot."),
            Tool("stress_darkness_add_minutes",
                "Debug: add minutes to today's home-evening timer (may credit evening).",
                Prop("minutes", "integer", "Minutes to add"),
                Prop("n", "integer", "Alias for minutes")),
            Tool("stress_darkness_complete_evening", "Debug: force-credit one evening (same-day duplicate blocked)."),
            Tool("stress_darkness_reset_therapy", "Debug: reset step1 evenings/timer flags (keeps active therapy)."),
            Tool("stress_darkness_sync", "Debug: DarknessService.SyncDarknessState + before/after snapshot."),
            Tool("stress_darkness_remission_status", "Read-only darkness remission/relapse risk snapshot."),
            Tool("stress_darkness_relapse_add", "Debug: add DarknessRelapseRisk while remission active.",
                Prop("amount", "integer", "Delta (can be negative)"),
                Prop("n", "integer", "Alias for amount")),
            Tool("stress_darkness_relapse_set", "Debug: set DarknessRelapseRisk 0–100.",
                Prop("amount", "integer", "Target risk"),
                Prop("value", "integer", "Alias for amount")),
            Tool("stress_darkness_remission_start", "Debug: mark cured and start remission (post-therapy state)."),
            Tool("stress_darkness_remission_clear", "Debug: clear remission and relapse risk."),
            Tool("stress_darkness_force_relapse", "Debug: force relapse (requires active remission)."),
            ...
            Tool("harvey_stress_social_start", "Debug: start Social anxiety treatment + therapy state."),
            Tool("harvey_stress_social_set_timer", "Debug: set Harvey proximity timer seconds.",
                Prop("seconds", "integer", "Target seconds (60 completes timer)")),
            Tool("harvey_stress_social_ready", "Debug: force ReadyToComplete for Social anxiety quest."),
